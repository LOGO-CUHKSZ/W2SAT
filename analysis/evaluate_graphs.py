...
def get_graph_metrics(formulas_path):
    title = [
        "num. vars",
        "num. clauses",
        "VIG clust.",
...
def getParser():
    parser = OptionParser(
        usage="usage: %prog [options] formula outfile", version="%prog 1.0"
    )
    parser.add_option(
        "-o", "--out", dest="out_file", default=None, help="save stats into a file"
    )
    parser.add_option(
        "-s",
        "--scale-free",
    ...
    for var in range(num_vars + 1)[1:]:
        if var in degrees:
            if var + num_vars in degrees:
                lst.append(
                    degrees[var] * 1.0 / (degrees[var] + degrees[var + num_vars])
                )
            else:
                lst.append(1.0)
        else:
...
def get_scale_free(source, scale_free=False):
    ...
    if scale_free:
        ...
        subprocess.call(
            [
                "/Users/anwu/Monkeyswedding/Projects/SAT_GAN/sat_gen/cpp/scalefree",
                source,
            ],
            stdout=f,
        )
    ...
    with open("blah.txt", "r") as f:
        for line in f.readlines():
            # or ("min. value" in line) or ("beta" in line) or ("max. error" in line):
            if "alpha" in line:
